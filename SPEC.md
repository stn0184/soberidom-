...
3. Кнопка «Печать/PDF» (браузерная печать со стилями @media print).
**Критерии приёмки:**
...
- Каждый проект обязан содержать этап с code='site_prep' (Этап 0 «Подготовка участка и быта»), sort=0, delivery_wave=1. Минимальный состав шагов Этапа 0 (шаблон, копируется в новый проект админ-кнопкой «Создать из шаблона»): план площадки; штабель пиломатериала (прокладки, продух, навес); раскроечный верстак из козелков; хранение инструмента; электричество на участке; вода/туалет; аптечка и СИЗ; правила распорядка и усталости.
...
Контент первого проекта (хозблок 3×4 как пилот наполнения) создаётся через админку после этапа 2 — параллельно разработке этапов 3–5.
